#临时改变powershell的执行策略为不受限制 
# powershell -Command "& {Set-ExecutionPolicy -ExecutionPolicy Unrestricted -Scope Process}"
$url = "https://stream-manager.yy.com/v3/channel/streams?uid=0&cid=54880976&sid=54880976&appid=0&sequence=1632705735370&encode=json"
$headers = @{
    "method"="POST"
...
while($flag)
{
    ...
    $stream_url = ($response.Content | ConvertFrom-Json).stream_url

    ffmpeg -i $stream_url -c copy -y $output_file -err_detect ignore_err -nostdin
